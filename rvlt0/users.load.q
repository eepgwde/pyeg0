/ 2019.10.19T03:09:02.647 j1 weaves
/ q users.load.q FILE [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-exit] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] 
/ q users.load.q FILE
/ q users.load.q
/ q users.load.q -help
FILE:LOADFILE:`$":cache/in/users.csv2"
o:.Q.opt .z.x;if[count .Q.x;FILE:hsym`${x[where"\\"=x]:"/";x}first .Q.x]
if[`help in key o;-1"usage: q users.load.q [FILE(default:cache/in/users.csv2)] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-af|asfile] [-exit]\n";exit 1]
SAVEDB:`:csvdb
SAVEPTN:`
...
LOADNAME:`users
SAVENAME:`users
LOADFMTS:"HHSCZBSEEIII"
LOADHDRS:`userid`yob`country`city`createddate`uscrypt0`plan`ntfctnmktpush`ntfctnmktemail`ncontacts`nrefls`nsrefls
