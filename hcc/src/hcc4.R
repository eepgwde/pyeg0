...
hcc0 <- list()
hcc0[["gam.enqs"]] <- gam0
hcc0[["wthr.enqs"]] <- wthr2enqs

rm(list=c("gam0", "wthr2enqs"))
...
hcc0[["discard"]] <- c("claims", "denqrs", "enqrs", hcc0[["outcomen"]])
...
## There's a limit of 6 on the formula. I use mm1 as the tensor key
## in paste1
ftres <- head(hcc0[["features"]], n=3)
ftres <- setdiff(ftres, "enqs")

descrs <- paste(sapply(ftres, function(x) 
    hcc.paste1(x, dr0="enqs", bs1="ps" ) ), collapse=" + ")
fmla <- as.formula(paste("value ~ ", descrs))
tag <- paste("te:", ftres, collapse=", ")
...
if (1==0) {
    write.csv(pdf1, file="pdf2-hcc4.csv", row.names=FALSE)
}

pdf2 <- read.csv("pdf2-hcc4.csv")
pdf2 <- tail(pdf2, 10)

t0 <- hcc0[["wthr.enqs"]]
idx <- which(colnames(t0) == "value")
colnames(t0)[idx] <- "enqs"             # *not* outcomen

t0 <- t0[, c("mm1", "dt0", "enqs"), drop=FALSE]
