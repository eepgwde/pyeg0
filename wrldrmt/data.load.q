/ 2019.08.11T14:05:29.056 peppy weaves
/ q data.load.q FILE [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-exit] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] 
/ q data.load.q FILE
/ q data.load.q
/ q data.load.q -help
FILE:LOADFILE:`$":data.csv"
o:.Q.opt .z.x;if[count .Q.x;FILE:hsym`${x[where"\\"=x]:"/";x}first .Q.x]
if[`help in key o;-1"usage: q data.load.q [FILE(default:data.csv)] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-af|asfile] [-exit]\n";exit 1]
SAVEDB:`:csvdb
SAVEPTN:`
...
JUSTSYMFMTS:" S  S SSSSSSSSSS   SSSSSSSSSS SSSSS S   S"
JUSTSYMHDRS:`classofworker`education`enrollineduinstlastwk`maritalstat`majorindustrycode`majoroccupationcode`race`hispanicorigin`sex`memberofalaborunion`reasonforunemployment`fullorparttimeemploymentstat`taxfilerstat`regionofpreviousresidence`stateofpreviousresidence`detailedhouseholdandfamilystat`detailedhouseholdsummaryinhousehold`migrationcodechangeinmsa`migrationcodechangeinreg`migrationcodemovewithinreg`liveinthishouse1yearago`migrationprevresinsunbelt`familymembersunder18`countryofbirthfather`countryofbirthmother`countryofbirthself`citizenship`fillincquestionnaireforveteransadmin`incomebinned
